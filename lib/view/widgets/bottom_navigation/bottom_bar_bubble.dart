...
class _BottomBarBubbleState extends State<BottomBarBubble>
  ...
  void _handleTextChangeFromOutside() {
    ...
    for (var i = 0; i < _iconCount; i++) {
      ...
      key.currentState?.updateSelect(false);
      iconsKey.add(key);
    }
  ...
  double _getLeftPosition() {
    ...
    if (widget.isArabic) {
      return ((widget.items.length - 1 - _selectedIndex) * iconWidth) +
          (iconWidth / 2);
    }
  ...
  Future _onChangeIndex(int index) async {
    ...
    iconsKey.forEach((element) {
      element.currentState?.updateSelect(false);
    });
